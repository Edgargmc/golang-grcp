...
const (
	...
	TodoService_SyncTodos_FullMethodName    = "/todo.TodoService/SyncTodos"
)
...
type TodoServiceClient interface {
	...
	SyncTodos(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[TodoChange, TodoEvent], error)
}
...
func (c *todoServiceClient) SyncTodos(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[TodoChange, TodoEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &TodoService_ServiceDesc.Streams[1], TodoService_SyncTodos_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[TodoChange, TodoEvent]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TodoService_SyncTodosClient = grpc.BidiStreamingClient[TodoChange, TodoEvent]
...
type TodoServiceServer interface {
	...
	// Bidirectional: client streams local changes, server streams back every
	// event (its own changes applied, plus everyone else's), on the same connection
	SyncTodos(grpc.BidiStreamingServer[TodoChange, TodoEvent]) error
...
func (UnimplementedTodoServiceServer) SyncTodos(grpc.BidiStreamingServer[TodoChange, TodoEvent]) error {
...
func _TodoService_SyncTodos_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TodoServiceServer).SyncTodos(&grpc.GenericServerStream[TodoChange, TodoEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TodoService_SyncTodosServer = grpc.BidiStreamingServer[TodoChange, TodoEvent]
